...
start_link(Args) ->
    ...
    start_link(Host, Port, Database, Password, ConnectTimeout).
...
start_link(Host, Port, Database, Password, ConnectTimeout) ->
    lager:debug("~s~n", [erlang:iolist_to_binary(["debugging: ~n"
                                                 , "\tHost = ", io_lib_pretty:print(Host), "\n"
                                                 , "\tPort = ", io_lib_pretty:print(Port), "\n"
                                                 ])]),
    gen_server:start_link(?MODULE, [Host, Port, Database, Password, ConnectTimeout], []).
...
init([Host, Port, Database, Password, ConnectTimeout]) ->
...
%% connection, try re-connect.
do_request(Req, From, #state{socket = undefined} = State) ->
    NewState = try_connect(State),
    case erlang:is_port(NewState#state.socket) of
        true ->
            do_request(Req, From, NewState);
        false ->
            {reply, {error, no_connection}, NewState}
    end;
do_request(Req, From, State) ->
...
%% connection, try re-connect.
do_pipeline(Pipeline, From, #state{socket = undefined} = State) ->
    NewState = try_connect(State),
    case erlang:is_port(NewState#state.socket) of
        true ->
...
try_connect(#state{host = Host,
                   port = Port,
                   connect_timeout = ConnectTimeout
                  } = State) ->
    lager:debug("im_redis_client try reconnect to server ~s:~p~n", [Host, Port]),
...
socket_opts() -> [binary, {active, once}, {packet, raw}, {reuseaddr, true}].
